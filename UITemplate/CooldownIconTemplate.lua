...
function HDH_AT_BaseCooldownTemplateMixin:SetHandler(startFunc, finishFunc)
...
function HDH_AT_CircleCooldownTemplateMixin:IsShownProgressTexture()
	return self.Progress.Texture:IsShown()
end
...
-----------------------------------------------
--- HDH_AT_BaseLinearCooldownTemplateMixin
-----------------------------------------------
HDH_AT_BaseLinearCooldownTemplateMixin = {}

function HDH_AT_BaseLinearCooldownTemplate_OnStarted(self)
	self.isRunning = true
	if self.OnStartedFunc then
		self.OnStartedFunc(self:GetParent())
	end
end

function HDH_AT_BaseLinearCooldownTemplate_OnFinished(self)
	self.startValue = nil
	self.duration = nil
	if self.isRunning and self.OnFinishedFunc then
		self.OnFinishedFunc(self:GetParent())
	end
	self.isRunning = false
end

function HDH_AT_BaseLinearCooldownTemplate_OnSizeChanged(self)
	local w, h = self:GetSize()
	local size = math.max(w, h)
	self.Progress.Texture:SetSize(size, size)
end

function HDH_AT_BaseLinearCooldownTemplate_OnUpdateLinearCooldown(self, elapsed)
...
function HDH_AT_BaseLinearCooldownTemplateMixin:SetCooldown(startValue, duration, isTimer)
	if self.startValue == startValue and self.duration == duration then return end
	self.startValue = startValue
	self.duration = duration
	self.isTimer = isTimer
	if self.isTimer then
		if self.OnStartedFunc then
			HDH_AT_BaseLinearCooldownTemplate_OnStarted(self)
		end
		self:SetScript('OnUpdate', HDH_AT_BaseLinearCooldownTemplate_OnUpdateLinearCooldown)
	else
		self:SetScript('OnUpdate', nil)
	end
end

function HDH_AT_BaseLinearCooldownTemplateMixin:SetValue(value)
	...
	if self.isTimer then
		if self.toFill then
			...
			elseif self.per < 1 then
				...
				if self.Progress:IsShown() then self.Progress:Hide() end
				HDH_AT_BaseLinearCooldownTemplate_OnFinished(self)
			end
		else
			if self.per < 1 then
				if not self.Progress:IsShown() then self.Progress:Show() end
	...
	if self.enableSpark and self.Spark then
...
function HDH_AT_BaseLinearCooldownTemplateMixin:Setup(w, h, cooldownType, toFill, sparkTexture, enableSpark, textureAlpha, texcoordRatio)
	if not texcoordRatio then
		texcoordRatio = 0.05
	end
	self:SetSize(w, h)
	...
	if cooldownType == DB.COOLDOWN_UP then
		...
		if enableSpark then
			self.Spark:SetHeight(4)
			self.Spark:SetPoint("BOTTOMLEFT", self.Progress, "BOTTOMLEFT", 0, 0)
			self.Spark:SetPoint("BOTTOMRIGHT", self.Progress, "BOTTOMRIGHT", 0, 0)
			self.Spark.Texture:SetTexCoord(0, 1, 0, 0.5 - texcoordRatio)
		end
	...
	elseif cooldownType == DB.COOLDOWN_DOWN then
		...
		if enableSpark then
			self.Spark:SetHeight(4)
			self.Spark:SetPoint("TOPLEFT", self.Progress, "TOPLEFT", 0, 0)
			self.Spark:SetPoint("TOPRIGHT", self.Progress, "TOPRIGHT", 0, 0)
			self.Spark.Texture:SetTexCoord(0, 1, 0.5 + texcoordRatio, 1)
		end
	...
	elseif cooldownType == DB.COOLDOWN_LEFT then
		...
		if enableSpark then
			...
			self.Spark:SetPoint("TOPRIGHT", self.Progress, "TOPRIGHT", 0, 0)
			self.Spark:SetPoint("BOTTOMRIGHT", self.Progress, "BOTTOMRIGHT", 0, 0)
			self.Spark.Texture:SetTexCoord(0, 0.5 - texcoordRatio, 0, 1)
		end
	...
	elseif cooldownType == DB.COOLDOWN_RIGHT then
		...
		if enableSpark then
			...
			self.Spark:SetPoint("TOPLEFT", self.Progress, "TOPLEFT", 0, 0)
			self.Spark:SetPoint("BOTTOMLEFT", self.Progress, "BOTTOMLEFT", 0, 0)
			self.Spark.Texture:SetTexCoord(0.5 + texcoordRatio, 1, 0, 1)
		end
...
function HDH_AT_BaseLinearCooldownTemplateMixin:SetTexture(texture)
	self.Progress.Texture:SetTexture(texture)
end

function HDH_AT_BaseLinearCooldownTemplateMixin:Stop()
	if self.Progress:IsShown() then
		self.Progress:Hide()
	end
...
function HDH_AT_BaseLinearCooldownTemplateMixin:Clear()
	self.startValue = nil
	self.duration = nil
	self.isTimer = nil
	self.per = nil
...
-- 	if self:GetParent().OnStartedFunc then
-- 		self.OnStartedFunc(self)
-- 	end
-- end
...
function HDH_AT_LinearCooldownTemplateMixin:SetHandler(startFunc, finishFunc)
	self.Progress:SetHandler(startFunc, finishFunc)
	self.Charge:SetHandler(startFunc, finishFunc)
end

function HDH_AT_LinearCooldownTemplateMixin:Setup(w, h, cooldownType, toFill, enableSpark, color, textureAlpha)
	if cooldownType == DB.COOLDOWN_UP or cooldownType == DB.COOLDOWN_DOWN  then
		self.Spark.Texture:SetTexture("Interface/AddOns/HDH_AuraTracker/Texture/ChargeSpark_v")
		self.Spark.Texture:SetVertexColor(unpack(color or {1,1,1,1}))
		self.ChargeSpark.Texture:SetTexture("Interface/AddOns/HDH_AuraTracker/Texture/ChargeSpark_v")
		self.ChargeSpark.Texture:SetVertexColor(unpack(color or {1,1,1,1}))
	elseif cooldownType == DB.COOLDOWN_LEFT or cooldownType == DB.COOLDOWN_RIGHT then
		self.Spark.Texture:SetTexture("Interface/AddOns/HDH_AuraTracker/Texture/ChargeSpark")
		self.Spark.Texture:SetVertexColor(unpack(color or {1,1,1,1}))
		self.ChargeSpark.Texture:SetTexture("Interface/AddOns/HDH_AuraTracker/Texture/ChargeSpark")
		self.ChargeSpark.Texture:SetVertexColor(unpack(color or {1,1,1,1}))
	end
	self.Progress:Setup(w, h, cooldownType, toFill, self.Spark, enableSpark, textureAlpha)
	self.Charge:Setup(w, h, cooldownType, toFill, self.ChargeSpark, true, textureAlpha, -0.15)
end

function HDH_AT_LinearCooldownTemplateMixin:SetTexture(texture)
	if self.Progress then
		self.Progress:SetTexture(texture)
		self.Overlay.Texture:SetTexture(texture)
	end
end

function HDH_AT_LinearCooldownTemplateMixin:IsShownProgressTexture()
	return self.Progress.Progress.Texture:IsShown()
end

function HDH_AT_LinearCooldownTemplateMixin:SetShownProgressTexture(bool)
	self.Progress.Progress.Texture:SetShown(bool)
end
...
function HDH_AT_LinearCooldownTemplateMixin:SetCooldown(startValue, duration, isTimer, isCharging)
	if not isCharging then
		self.Progress:SetCooldown(startValue, duration, isTimer)
	else
		self.Charge:SetCooldown(startValue, duration, isTimer)
	end
end
...
function HDH_AT_CooldownIconTemplateMixin:UpdateCooldowning(bool)
	...
	if bool then
		...
		if not self.Cooldown:IsShownProgressTexture() then
